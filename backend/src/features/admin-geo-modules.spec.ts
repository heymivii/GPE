...
describe('CityService', () => {
  ...
  it('returns all cities', async () => {
    cityRepo.find.mockResolvedValue([{ idCity: 1, name: 'Paris' }]);
    const result = await service.findAll();
    expect(result).toHaveLength(1);
  });
...
describe('ContinentService', () => {
  ...
  it('returns all continents', async () => {
    continentRepo.find.mockResolvedValue([{ idContinent: 1, name: 'Europe' }]);
    const result = await service.findAll();
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('Europe');
  });
  ...
  it('creates a continent', async () => {
    const c = { idContinent: 1, name: 'Oceanie', isoCode: 'OC' };
    continentRepo.create.mockReturnValue(c);
    continentRepo.save.mockResolvedValue(c);
    const result = await service.create({ name: 'Oceanie', isoCode: 'OC' });
    expect(result.name).toBe('Oceanie');
  });
});
