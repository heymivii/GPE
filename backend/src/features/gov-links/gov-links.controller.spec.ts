...
const mockRun = (id: number, cc: string) => ({ id, countryCode: cc, status: 'running', total: 11, results: [] });
...
describe('GovLinksController', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    ctrl = new GovLinksController(mockService as never, mockOrchestrator as never);
  });
  ...
  it('generate-country throws 400 for unsupported country', async () => {
    await expect(ctrl.generateCountry('XX')).rejects.toThrow(BadRequestException);
  ...
  it('rerunCategory throws 400 for unknown category', async () => {
    await expect(ctrl.rerunCategory('7', 'nope')).rejects.toThrow(BadRequestException);
  ...
  it('rerunCategory throws 400 when run not found', async () => {
    ...
    await expect(ctrl.rerunCategory('999', 'visa')).rejects.toThrow(BadRequestException);
  });
  ...
  it('rerunCategory delegates with run.countryCode', async () => {
    ...
    expect(mockOrchestrator.rerunCategory).toHaveBeenCalledWith(7, 'FR', 'visa');
    expect(result).toEqual(mockRun(7, 'FR'));
  });
});
