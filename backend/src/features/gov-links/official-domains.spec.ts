...
describe('isOfficialDomain', () => {
  it('accepts an official FR domain', () => {
    expect(
      isOfficialDomain('https://france-visas.gouv.fr/en/etudiant', 'FR'),
    ).toBe(true);
    expect(isOfficialDomain('https://www.service-public.fr/x', 'FR')).toBe(
      true,
    );
  });
  it('rejects a non-official domain', () => {
    expect(isOfficialDomain('https://blog-immigration.com/visa', 'FR')).toBe(
      false,
    );
  });
